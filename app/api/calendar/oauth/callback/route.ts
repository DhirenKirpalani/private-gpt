...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/channels?calendar=connected`)
  } catch (err: any) {
    console.error("[CALENDAR CALLBACK]", err)
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/channels?error=${encodeURIComponent(err?.message || "Calendar OAuth callback failed")}`)
  }
}
