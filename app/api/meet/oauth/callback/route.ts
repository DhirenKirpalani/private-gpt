...
export async function GET(req: NextRequest) {
  try {
    ...
    // Upsert Meet connection (reuse calendar_connections table with provider="googlemeet")
    const { data: existing } = await supabase
      .from("calendar_connections")
      .select("id")
      .eq("user_id", userId)
      .eq("provider", "googlemeet")
      .maybeSingle()
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/channels?meet=connected`)
  } catch (err: any) {
    console.error("[MEET CALLBACK]", err)
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/channels?error=${encodeURIComponent(err?.message || "Meet OAuth callback failed")}`)
  }
}
